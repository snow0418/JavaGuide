...
3. 帮助服务器避免排序和临时表。(ORDER BY Optimizaiton & GROUP BY Optimization with indices)
4. 将随机IO变为顺序IO
...
(以InnoDB为例说明：
	1. 不是所有的列都适合做索引，比如，太长的键不适合做索引,这样会导致索引文件非常大（增加内存开销？）; 
	2. 时间和空间上的开销，以及对数据库性能的影响；
	3. 索引维护的开销可以结合InnoDB的二级索引是存储的主键的值来避免频繁的索引维护，进而避免这个索引维护的开销;
)
